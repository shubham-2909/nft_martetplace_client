...
export default function MyNFTs() {
  ...
  async function resellItem(tokenId, tokePrice) {
    try {
      ...
      const price = ethers.parseUnits(tokePrice, 'ether')
      const transaction = await nftMarketContract.resellToken(tokenId, price, {
        value: listingPrice,
      })
  ...
  if (loading === 'loaded' && !myNfts.length) {
    return (
      <div className='container mx-auto px-4 mt-10 pt-16'>
        <h1 className='text-3xl font-semibold mb-8 ml-5'>No NFTs Owned</h1>
      </div>
    )
  }
  return (
    <div className='container mx-auto px-4 mt-10 pt-16'>
      <h1 className='text-3xl font-semibold mb-8 ml-5'>My NFTs</h1>

      <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8'>
        {myNfts.map((nft) => (
          <div
            key={nft.name}
            className='border p-4 rounded-lg shadow-md hover:shadow-xl'
          >
            <Image
              src={nft.image}
              alt={nft.name}
            ...
            <div className='flex justify-between items-center'>
              ...
              <button
                ...
                onClick={() => resellItem(nft.tokenId, nft.price)}
              >
                Resell
              </button>
            </div>
